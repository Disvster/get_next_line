...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (ft_free_stash(&stash[fd], NULL, 0));
	bread = 1;
	line = NULL;
	buffer = NULL;
	while (bread > 0 && find_line(stash[fd]))
	{
		...
		if (bread > 0)
			stash[fd] = ft_stash_copycat(stash[fd], &buffer);
		else
			free (buffer);
		if (!stash[fd] || bread == -1)
			return (ft_free_stash(&stash[fd], NULL, bread));
	}
	line = ft_trimstash(stash[fd]);
	return (ft_free_stash(&stash[fd], line, bread));
}
